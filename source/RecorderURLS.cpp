...
bool RecorderURLS::GetNextURL( XString& url )
{
    ...
    if( _firstRequest )
        _nextRequest = XTime::FromISOExtString( _startTime ).ToUnixTimeAsMSecs();

    int64_t exportEnd = XTime::FromISOExtString( _endTime ).ToUnixTimeAsMSecs();
    int64_t requestStart = _nextRequest;
    int64_t requestEnd = _nextRequest + _requestSize;
...
float RecorderURLS::PercentComplete() const
{
    if( _firstRequest )
        return 0.0;

    int64_t exportStart = XTime::FromISOExtString( _startTime ).ToUnixTimeAsMSecs();
    int64_t exportEnd = XTime::FromISOExtString( _endTime ).ToUnixTimeAsMSecs();

    return (float)(_nextRequest - exportStart) / (float)(exportEnd - exportStart);
}
